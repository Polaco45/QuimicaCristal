...
class CompleteInstitutionalQualification(AgentTool):
    ...
    def _do_atomic_close(self, env, partner_id, data):
        ...
        # FIX v1.9.8: la opp apunta al CONTACTO (subcontacto con mobile/email),
        # no a la empresa madre. Así templates/cotizaciones encuentran a quién
        # mandar el mensaje. La empresa queda como commercial_partner_id (auto,
        # por parent_id del contacto) — las facturas salen a nombre de ella.
        opp_company_name = company.name if company else contact.name
        opp_name = f"[Plan Control] {opp_company_name}"
        opp_description = self._build_lead_description(data, company or contact, contact)

        # Buscar opp abierta del CONTACTO (no de la empresa, porque puede haber
        # varios contactos de la misma empresa con opps distintas)
        ...
        if existing_opp:
            ...
            opp = Lead.create({
                'name': opp_name,
                'partner_id': contact.id,  # ← contacto, no empresa madre
                'contact_name': contact_name,
                'function': rol,
        ...
        activity_note = (
            f"<p><b>🎯 Coordinar visita Plan Control — {opp_company_name}</b></p>"
        ...
        activity = Activity.create({
            ...
            'summary': f"Coordinar visita Plan Control: {opp_company_name}",
            'note': activity_note,
